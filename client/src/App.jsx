...
function App() {
    // URL-based mode detection: /govuk skips intro and goes straight to GOV.UK mode
    const isGovukPath = window.location.pathname.toLowerCase().includes('govuk') ||
        window.location.hash.toLowerCase().includes('govuk');

    const [state, setState] = useState(null);
    const [initialState, setInitialState] = useState(null);
    const [connectionStatus, setConnectionStatus] = useState('connecting');
    const [showOnboarding, setShowOnboarding] = useState(!isGovukPath); // Skip intro if /govuk
    const [theme, setTheme] = useState('dark');
    const [crisisActive, setCrisisActive] = useState(false);
    const [achievementToast, setAchievementToast] = useState(null);
    const [viewMode, setViewMode] = useState(isGovukPath ? 'govuk' : 'cinematic'); // Start in govuk if /govuk path
